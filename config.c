...
static void read_config_file(FILE *config, char **strings, int *frame_width,
  ...
  int i, op, len;
  unsigned int state,
      xbuttons[] = {Button1, Button2, Button3, Button4, Button5};
  ...
  while (fgets(line, sizeof(line), config) != NULL) {
    ...
    case BUTTON_OP_KEY:
      ...
      if (line[0] == KEY_OP_KEY) {
        if ((keys[defined_keys].keycode =
                 XKeysymToKeycode(d, XStringToKeysym(p))) == 0)
          break;
      ...
      fprintf(ERR, "Registered %u+%u (%s) %c.\n", state,
              (line[0] == KEY_OP_KEY) ? keys[defined_keys - 1].keycode
                                      : buttons[defined_buttons - 1].button,
              p, OP_SUB_KEYS[op]);
#endif
      continue;
    case FONT_KEY:
    ...
    case OTHER_OPTION_KEY:
      ...
      fprintf(
          ...
          options, options, ((options & FOCUS_WHEN_RAISED) ? '+' : ' '),
          FOCUS_WHEN_RAISED, ((options & FOCUS_WHEN_CIRCULATED) ? '+' : ' '),
          FOCUS_WHEN_CIRCULATED, ((options & FOCUS_WHEN_MAPPED) ? '+' : ' '),
          FOCUS_WHEN_MAPPED, ((options & RAISE_WHEN_CIRCULATED) ? '+' : ' '),
...
int configure_heliwm(char *config_file_name) {
  ...
  char *given[NUM_VALUES] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL},
       *def[NUM_VALUES] = {DEF_TEXT_COLOR,   DEF_FRAME_COLOR,  DEF_BORDER_COLOR,
                           DEF_SHADE0_COLOR, DEF_SHADE1_COLOR, DEF_BAND_COLOR,
                           DEF_FONT};
#ifdef RCPARSER
  FILE *config;
  char *name;
  ...
  if (strncmp(config_file_name, "~/", 2) == 0) {
    ...
    if ((home = getenv("HOME")) == NULL)
      home = ".";
    if ((name = malloc(strlen(home) + strlen(config_file_name))) == NULL)
  ...
  if (defined_keys == 0) {
    for (i = 0; i < predefined_keys; i++) {
      ...
      fprintf(ERR, "Registering %u+%u (%s) %c.\n", keys[i].state,
              (unsigned int)keys[i].keysym, XKeysymToString(keys[i].keysym),
...
#ifdef NLS
  {
    ...
    fontset = XCreateFontSet(d, NAME(FONT), &missing, &nmissing, &defstr);
    if (nmissing > 0) {
  ...
  bgc = alloc_color(&col, NAME(BACKGROUND)) ? BlackPixel(d, scr) : col.pixel;
  ...
  band_gc = XCreateGC(d, rootwin,
                      GCFunction | GCForeground | GCLineWidth | GCLineStyle |
                          GCCapStyle | GCJoinStyle | GCSubwindowMode,
                      &val);

#ifdef FRAME
  val.foreground =
      alloc_color(&col, NAME(SHADE_UPPER_LEFT)) ? white : col.pixel;
  frame0_gc = XCreateGC(d, rootwin,
                        GCForeground | GCLineWidth | GCLineStyle | GCCapStyle |
                            GCJoinStyle,
                        &val);
  val.foreground =
      alloc_color(&col, NAME(SHADE_LOWER_RIGHT)) ? white : col.pixel;
  frame1_gc = XCreateGC(d, rootwin,
                        GCForeground | GCLineWidth | GCLineStyle | GCCapStyle |
                            GCJoinStyle,
                        &val);
#endif
